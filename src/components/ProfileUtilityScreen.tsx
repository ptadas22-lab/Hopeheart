...
export default function ProfileUtilityScreen({ 
  ...
  // Settings & Customizations States
  const [settingsTab, setSettingsTab] = useState<'control' | 'transparency'>('control');
  const [theme, setTheme] = useState(() => localStorage.getItem('hopeheart_theme') || 'light');
  const [dailyBrief, setDailyBrief] = useState(() => localStorage.getItem('hopeheart_daily_brief_enabled') !== 'false');

  const handleDailyBriefToggle = () => {
    const next = !dailyBrief;
    setDailyBrief(next);
    localStorage.setItem('hopeheart_daily_brief_enabled', next ? 'true' : 'false');
  };
  ...
  const dataProtectionLocalStorageKeys = [
    ...
    'hopeheart_trusted_contacts',
    'hopeheart_emergency_contacts',
    'hopeheart_safety_plan',
    'hopeheart_support_preferences',
    'hopeheart_mood_history',
    'hopeheart_gratitude',
    'hopeheart_positive_memories',
    'hopeheart_favorite_resources',
    'hopeheart_remember_comfort_activity',
    'hopeheart_remember_survival_memory',
    'hopeheart_daily_brief_enabled',
    'hopeheart_profile_display_name',
    'hopeheart_profile_avatar',
    'hopeheart_profile_age_group',
    'hopeheart_profile_gender',
    'hopeheart_profile_profession',
    'hopeheart_profile_language',
    'hopeheart_profile_support_interest',
    'hopeheart_profile_best_quality',
    'hopeheart_profile_nature',
    'hopeheart_profile_vibe',
    'hopeheart_theme'
  ];

  const handleViewStoredData = () => {
    const storedDataSummary = dataProtectionLocalStorageKeys
      .map((key) => `${key}: ${localStorage.getItem(key) === null ? 'Not saved' : 'Saved'}`)
      .join('\n');
    alert(`HopeHeart local data on this device:\n\n${storedDataSummary}`);
  };

  const handleExportData = () => {
    const exportObj: Record<string, string> = {};
    dataProtectionLocalStorageKeys.forEach((key) => {
      const val = localStorage.getItem(key);
      if (val !== null) {
        exportObj[key] = val;
      }
    });
    // Add any hopebuddy_ keys
    for (let i = 0; i < localStorage.length; i++) {
      const key = localStorage.key(i);
      if (key && key.startsWith('hopebuddy_')) {
        const val = localStorage.getItem(key);
        if (val !== null) {
          exportObj[key] = val;
        }
    ...
    downloadAnchor.setAttribute("download", `hopeheart_backup_${new Date().toISOString().split('T')[0]}.json`);
    ...
    reader.onload = (e) => {
      try {
        ...
        Object.keys(imported).forEach((key) => {
          if (key.startsWith('hopeheart_') || key.startsWith('hopebuddy_')) {
            localStorage.setItem(key, imported[key]);
            count++;
          }
        });

        alert(`Successfully imported ${count} items from backup! HopeHeart will now reload to apply changes.`);
        window.location.reload();
      } catch (err) {
        alert("Invalid backup file format. Please upload a valid JSON backup file generated by HopeHeart.");
      }
    };
  ...
  const handleDeleteLocalData = () => {
    ...
    // Also remove any hopebuddy_ keys
    ...
    alert('Local HopeHeart data was successfully cleared from this device. The page will reload.');
    window.location.reload();
  };
        ...
        <AnimatePresence mode="wait">
          ...
          {subStage === 'privacy' && (
            <motion.div
              ...
              animate={{ opacity: 1, scale: 1 }}
              exit={{ opacity: 0 }}
              className="space-y-5 max-w-md mx-auto mt-1 sm:mt-2 select-none"
            >
              {/* Tab Selector */}
              <div className="flex bg-[#FFF3E4]/50 border border-orange-100/60 rounded-2xl p-1 gap-1">
                <button
                  type="button"
                  onClick={() => setSettingsTab('control')}
                  className={`flex-1 py-2 text-[12.5px] font-display font-black rounded-xl transition-all cursor-pointer ${
                    settingsTab === 'control'
                      ? 'bg-[#FF7527] text-white shadow-xs'
                ...
                <button
                  type="button"
                  onClick={() => setSettingsTab('transparency')}
                  className={`flex-1 py-2 text-[12.5px] font-display font-black rounded-xl transition-all cursor-pointer ${
                    settingsTab === 'transparency'
                      ? 'bg-[#FF7527] text-white shadow-xs'
                      : 'text-gray-500 hover:text-gray-700'
                  }`}
                >
              ...
              {settingsTab === 'control' ? (
                <div className="space-y-4">
                  ...
                  <div className="hh-surface rounded-2.5xl p-4.5 space-y-3.5 text-left">
                    <h4 className="font-display font-black text-gray-800 text-[14px] flex items-center gap-1.5 border-b border-gray-100 pb-2">
                      🎨 Appearance Theme
                    </h4>
                      ...
                      {[
                        ...
                        return (
                          <button
                            ...
                            onClick={() => {
                              setTheme(t.id);
                              localStorage.setItem('hopeheart_theme', t.id);
                              document.documentElement.setAttribute('data-theme', t.id);
                            }}
                            className={`py-2 px-2.5 rounded-xl border flex flex-col items-center justify-center gap-1 transition-all active-scale cursor-pointer ${
                              active
                                ? 'bg-orange-50 border-[#FF7527] text-[#FF7527] font-bold shadow-3xs'
                                : 'bg-white border-gray-250 text-gray-600 hover:bg-gray-50'
                            }`}
                          >
                            <span className="text-[18px]">{t.icon}</span>
                            <span className="text-[11.5px] font-display">{t.label}</span>
                          </button>
                        );
                      })}
                    </div>
                  ...
                  <div className="hh-surface rounded-2.5xl p-4.5 space-y-4 text-left">
                    ...
                    <div className="flex items-center justify-between pb-3.5 border-b border-gray-150/40">
                      <div className="space-y-0.5 max-w-[75%]">
                        <span className="text-[13px] font-bold text-gray-800 block">👤 Show profile basics</span>
                    ...
                    <div className="flex items-center justify-between pb-3.5 border-b border-gray-150/40">
                      <div className="space-y-0.5 max-w-[75%]">
                        <span className="text-[13px] font-bold text-gray-800 block">🧡 Show mood status</span>
                    ...
                    {/* Toggle 4: Approximate Nearby */}
                    <div className="flex items-center justify-between pb-3.5 border-b border-gray-150/40">
                      <div className="space-y-0.5 max-w-[75%]">
                        <span className="text-[13px] font-bold text-gray-800 block">📍 Use approximate nearby</span>
                        <p className="text-[11px] text-gray-455 font-semibold leading-normal">
                    ...
                    {/* Toggle 5: Hide from suggestion list */}
                    <div className="flex items-center justify-between">
                      <div className="space-y-0.5 max-w-[75%]">
                        <span className="text-[13px] font-bold text-gray-800 block">🔒 Hide from suggestions</span>
                        <p className="text-[11px] text-gray-455 font-semibold leading-normal">
                  ...
                  <div className="hh-surface rounded-2.5xl p-4.5 space-y-3.5 text-left">
                    ...
                    <div className="flex items-center justify-between">
                      <div className="space-y-0.5 max-w-[75%]">
                        <span className="text-[13px] font-bold text-gray-800 block">Daily Wellness Briefing</span>
                        <p className="text-[11px] text-gray-450 font-semibold leading-normal">
                          Send a quiet on-screen notification summary at 9:00 AM.
                        </p>
                      </div>
                      <button
                        onClick={handleDailyBriefToggle}
                        type="button"
                        className={`w-11 h-6 rounded-full p-0.5 transition-colors cursor-pointer shrink-0 ${dailyBrief ? 'bg-[#FF7527]' : 'bg-gray-200'}`}
                      >
                        <div className={`w-5 h-5 rounded-full bg-white shadow-sm transition-transform transform ${dailyBrief ? 'translate-x-5' : 'translate-x-0'}`} />
                      </button>
                    </div>
                  </div>
                  ...
                  <div className="hh-surface rounded-2.5xl p-4.5 space-y-4.5 text-left">
                    <h4 className="font-display font-black text-gray-800 text-[14px] flex items-center gap-1.5 border-b border-gray-100 pb-2">
                      💾 Local Data Protection
                    </h4>
                    
                    <p className="text-[11.5px] text-gray-500 font-semibold leading-relaxed">
                      All your check-ins, journal entries, safety plans, and contacts are saved strictly on this device unless you choose to share them.
                    </p>
                    ...
                    <div className="space-y-2.5">
                      <button
                        ...
                        className="w-full py-2.5 bg-white hover:bg-gray-50 border border-gray-250 text-gray-700 font-display font-extrabold text-[12.5px] rounded-xl cursor-pointer transition-all active-scale"
                      >
                        🔍 View Active Storage Audit
                      </button>

                      <button
                        type="button"
                        onClick={handleExportData}
                        className="w-full py-2.5 bg-white hover:bg-gray-50 border border-gray-250 text-gray-700 font-display font-extrabold text-[12.5px] rounded-xl cursor-pointer transition-all active-scale flex items-center justify-center gap-1.5"
                      >
                      ...
                      <div className="relative">
                        ...
                        <button
                          type="button"
                          className="w-full py-2.5 bg-white hover:bg-gray-50 border border-gray-250 text-gray-700 font-display font-extrabold text-[12.5px] rounded-xl transition-all active-scale flex items-center justify-center gap-1.5"
                        >
                          📤 Restore Data Backup
                        </button>
                      </div>
                      ...
                      <button
                        type="button"
                        onClick={() => setShowDeleteLocalDataModal(true)}
                        className="w-full py-2.5 bg-[#FEF6F5] hover:bg-[#FCECEB] border border-red-200 text-red-700 font-display font-extrabold text-[12.5px] rounded-xl cursor-pointer transition-all active-scale"
                      >
                      ...
                      {onRequestAccountDataDeletion && (
                        <button
                          type="button"
                          onClick={onRequestAccountDataDeletion}
                          className="w-full py-2.5 bg-[#FF7527] hover:bg-[#E55D13] text-white font-display font-black text-[12.5px] rounded-xl cursor-pointer transition-all active-scale shadow-3xs"
                        >
                ...
                <div className="space-y-4 max-h-[480px] overflow-y-auto pr-1">
                  {/* Privacy Policy */}
                  <div className="bg-white border border-[#EDE9DE] rounded-2xl p-4 text-left space-y-2.5 shadow-3xs">
                    <h5 className="font-display font-black text-gray-800 text-[13.5px] flex items-center gap-1.5">
                      🔒 Privacy Policy
                    </h5>
                    <p className="text-[11.5px] text-gray-600 font-semibold leading-relaxed">
                      HopeHeart is built with privacy-first standards. None of your logs, check-ins, or coping strategies are synced to cloud backends unless you explicitly consent to sharing them. No tracking SDKs or ad pixels are embedded.
                    </p>
                  </div>
                  ...
                  <div className="bg-white border border-[#EDE9DE] rounded-2xl p-4 text-left space-y-2.5 shadow-3xs">
                    ...
                    <p className="text-[11.5px] text-gray-600 font-semibold leading-relaxed">
                      We offer a safe peer-support space. Do not post clinical diagnoses, medical advice, prescriptions, or trigger comments. Treat other members with kindness, empathy, and absolute confidentiality.
                    </p>
                  </div>
                  ...
                  <div className="bg-white border border-[#EDE9DE] rounded-2xl p-4 text-left space-y-2.5 shadow-3xs">
                    ...
                    <p className="text-[11.5px] text-gray-600 font-semibold leading-relaxed">
                      HopeHeart is strictly for peer-to-peer emotional comfort and support. It does not replace professional therapy, clinical consultation, psychiatric evaluation, or crisis intervention lines.
                    </p>
                  </div>
                  ...
                  <div className="bg-white border border-[#EDE9DE] rounded-2xl p-4 text-left space-y-2.5 shadow-3xs">
                    ...
                    <div className="space-y-1.5 text-[11.5px] text-gray-500 font-semibold leading-normal">
                      <div><strong className="text-gray-700">App Version:</strong> 2.1.0</div>
                      <div><strong className="text-gray-700">License:</strong> MIT Open Source</div>
                      <div><strong className="text-gray-700">Copyright:</strong> © 2026 HopeHeart Project.</div>
                      <p className="text-[10px] text-gray-400 pt-1 leading-relaxed">
                        Built with React, Vite, Framer Motion, and Tailwind CSS.
                      </p>
                    </div>
                  </div>
                </div>
              ...
              <div className="grid grid-cols-2 gap-3 pt-2">
                <button
                  ...
                  className="w-full py-2.5 bg-white hover:bg-gray-50 border border-gray-250 text-gray-700 font-display font-extrabold text-[13px] rounded-xl cursor-pointer transition-all active-scale"
                >
                  Close
                </button>
                <button
                  type="button"
                  onClick={() => {
                    alert("Settings and configurations updated successfully.");
                    setSubStage('profile');
                  }}
                  className="w-full py-2.5 bg-[#FF7527] hover:bg-[#E55D13] text-white font-display font-black text-[13px] rounded-xl cursor-pointer shadow-xs transition-all active-scale"
                >
                  Save Settings
                </button>
              </div>
